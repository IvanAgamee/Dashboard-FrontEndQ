  ...
  <q-dialog v-model="modalContenido" persistent>
    <q-card style="min-width: 350px">
      ...
      <q-card-actions align="right" class="text-primary">
        ...
        <q-btn @click="crearContenido()" flat label="Agregar Elemento" v-close-popup></q-btn>
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
...
const agregarSeccion = async () => {
  ...
  if(objObjetos.value.length>0) {
    const idSeccion = await apiSeccion.getLastIdSeccion();
...
const validarInputInfoGral = () => {
  if(selectedPrograma==null || objSeccion.value.nombre == '' || objSeccion.value.descripcion == '' || objModulo.value == null){
...
const validarGeneral = () => {
  if(tieneContenido==true && selectedPrograma==null && objSeccion.value.objeto.length==0){
